...
export default function Navbar() {
  ...
  const handleLogout = () => {
    ...
    navigate("/home");
    window.location.reload(); // reset navbar state
  };
        ...
        <div className="hidden md:flex items-center gap-6">
          ...
          {!token ? (
            ...
            <div className="flex items-center gap-2 border border-slate-200 rounded-lg px-4 py-2 bg-gray-50">
              ...
              <button onClick={handleLogout} className="ml-4 bg-red-500 text-white px-3 py-1 rounded hover:bg-red-600 transition">
                Logout
              </button>
            </div>
          )}
          ...
          <div className="flex flex-col px-6 py-4 gap-3">
            ...
            {!token ? (
              ...
              <div className="flex items-center gap-2 border border-slate-200 rounded-lg px-4 py-2 bg-gray-50">
                <HiUser className="w-5 h-5 text-gray-700" />
                <span className="font-medium">Hi {name}</span>
                <button onClick={handleLogout} className="ml-2 bg-red-500 text-white px-2 py-1 rounded hover:bg-red-600 transition">
                  Logout
                </button>
              </div>
            )}
